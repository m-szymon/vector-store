...
def parse_args(argv=None):
    ...
    p.add_argument("--reset", action="store_true", help="Drop table first (full reset) before creating schema")
...
def main():
    ...
    if not skip_schema:
        if args.reset:
            cmd = [
                ...
                "-P", "schema_cleanup=true",
            ]
            run_latte(cmd, "SCHEMA RESET")
        cmd = [
            latte_bin, "schema", str(workload), args.node,
            "-P", f'keyspace="{args.keyspace}"',
            "-P", f'table="{args.table}"',
    ...
    for sname, sdata in search_phases.items():
        ...
        if rc:
            ...
            if rp50 is not None:
                parts.append(f"p50={rp50:.0f}")
            if rp99 is not None:
                parts.append(f"p99={rp99:.0f}")

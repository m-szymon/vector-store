...
def write_tsv(rows, path, fields):
    ...
    with open(path, "w") as f:
        for row in rows:
            f.write("\t".join(row.get(f, "") for f in fields) + "\n")
...
def bucket_terms(doc_freq, total_docs):
    rare = []
    medium = []
    common = []
    for term, freq in doc_freq.items():
        if len(term) < 2 or term in TANTIVY_ENGLISH_STOP_WORDS:
            continue
        ratio = freq / total_docs
        if ratio <= 0.01:
            rare.append(term)
        elif ratio <= 0.10:
            medium.append(term)
...
def generate_synthetic_queries(documents, count_per_bucket=5,
    ...
    rare, medium, common = bucket_terms(doc_freq, total_docs)
...
def parse_args(argv=None):
    ...
    p.add_argument("--max-docs", type=int, default=None, help="Cap total documents before scaling")
...
def main():
    ...
    if args.scale_mode == "synthetic":
        ...
        documents, applied_scale = scale_synthetic(
            documents,

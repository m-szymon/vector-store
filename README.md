...
| `VECTOR_STORE_MTLS_CA_CERT_PATH`           | When set, the mTLS API endpoint is enabled. Path to the CA certificate bundle used to verify client certificates; requires VECTOR_STORE_TLS_CERT_PATH and VECTOR_STORE_TLS_KEY_PATH. |                          |
...
| `VECTOR_STORE_SCYLLADB_CERTIFICATE_FILE`   | The path to a TLS certificate file for ScyllaDB authentication. Can be used alone or with username/password.                                                                         |                          |
| `VECTOR_STORE_DISABLE_COLORS`              | Disable ANSI colors in log output (`true`/`false`). Colors are also disabled when stdout is not a terminal.                                                                          | `false`                  |
...
| `VECTOR_STORE_MEMORY_LIMIT`                | How much available memory (in bytes) could be in use to allow allocation more memory for the index.                                                                                  | avail mem - safety buffer|
...
| `VECTOR_STORE_CQL_PREFERRED_DATACENTER`    | Preferred ScyllaDB datacenter for the CQL driver's load balancing policy. Falls back to other datacenters if unavailable.                                                            |                          |
| `VECTOR_STORE_CQL_PREFERRED_RACK`          | Preferred ScyllaDB rack within `VECTOR_STORE_CQL_PREFERRED_DATACENTER` for the CQL driver's load balancing policy. Requires the datacenter variable to also be set.                  |                          |
...
| `VECTOR_STORE_DISKANN_BACKEND`             | Use DiskANN as the indexing engine instead of USearch, on the given data provider: `inmem` (vectors and adjacency in RAM) or `scylla` (exact vectors read back from the base table). | (unset: USearch)         |
...
| `VECTOR_STORE_FULLTEXT_INDEXES`            | Serve full-text (`fulltext_index`) indexes. When disabled they are ignored and left to other nodes.                                                                                  | `true`                   |
| `VECTOR_STORE_SUBSTRING_INDEXES`           | Serve substring (`substring_index`) indexes. When disabled they are ignored and left to other nodes.                                                                                 | `true`                   |

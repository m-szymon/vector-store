...
mod tests {
    ...
    fn extract_from_cql_vector() {
        ...
        assert_eq!(result, Vector(vec![1.0, 2.5, 3.0]));
    }
    ...
    fn extract_from_dynamodb_json_blob() {
        ...
        let result = Vector::try_from(value).unwrap();
        assert_eq!(result, Vector(vec![123.4, 234.5, 345.6]));
    }
    ...
    fn extract_from_attrs_map_with_blob_keys() {
        ...
        .unwrap();
        assert_eq!(result, Some(Vector(vec![1.0, 2.0])));
    }
    ...
    fn extract_from_attrs_map_with_text_keys() {
        ...
        .unwrap();
        assert_eq!(result, Some(Vector(vec![3.0])));
    }
